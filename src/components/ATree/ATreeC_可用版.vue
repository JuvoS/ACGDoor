...
export default {
  mounted() {
    ...
    function initTree() {
      ...
      //tree容器宽
      const treeWidth = basicContainer.offsetWidth - margin.left - margin.right;
      //tree容器高
      const treeHeight =
        basicContainer.offsetHeight - margin.top - margin.bottom;
      // 中心点XY
      ...
      initTreeCell(treeContainer, relationData, { x: centralX, y: centralY });
    }
    ...
    function updateTree(source, { x, y }) {
      ...
      nodes.forEach((d) => {
        ...
        d.y = (d.y ? d.y : 0) + y - minCellWidth / 2;
        d.x = (d.x ? d.x : 0) + x - minCellWidth / 3;
      });
    ...
    let rootRelation;
    function initTreeCell(container, treeRelation, { x, y }) {
      createTreeRootNode(
        container,
        { x: x - minCellWidth * 1.5, y },
        treeRelation
      );
