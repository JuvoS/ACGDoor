...
export default {
  ...
  methods: {
    ...
    treeInit() {
      ...
      const treeWidth =
        document.getElementById("treeRoot").offsetWidth -
        margin.left -
        margin.right; //tree容器宽
      const treeHeight =
        document.getElementById("treeRoot").offsetHeight -
        margin.top -
        margin.bottom; //tree容器高
    ...
    //数据处理
    dealData() {
      const temp = d3.hierarchy(treeData);
      temp.x0 = this.centralPoint[0];
      temp.y0 = this.centralPoint[1];
      temp.descendants().forEach((d) => {
        d._children = d.children; //添加_children属性，用于实现点击收缩及展开功能
        d.id = "cell" + this.uuid(); //绑定唯一标识ID
      });
      this.root = temp;
      this.update(temp, {
    ...
    //画根节点
    drawRoot(container, { x, y }, basicName) {
      const title = container
        .append("g")
        .attr("id", "rootTitle")
        .attr("transform", `translate(${y},${x})`);
      const rootNodeLength = basicName[0].length * this.fontSize + 30;
      title
        .append("svg:rect")
    ...
    update(source, { x, y }) {
      ...
      nodes.forEach((d) => {
        ...
        d.y = dirRight * (d.y + this.rootNodeLength / 2) + y;
        d.x = d.x + x;
      });
      ...
      const nodeEnter = node
        ...
        .on("click", (d) => {
          d.depth !== 0 && this.clickNode(d, { x, y }); //根节点不执行点击事件
        });
      ...
      nodeEnter.each((d) => {
        ...
        if (d.depth > 0 && d._children) {
          ...
          d3.select(`#g${d.id} text`)
            ...
            .attr(
              "x",
              right ? xDistance - this.circleR : xDistance + this.circleR
            ); //修改文本属性
