...
export default {
  ...
  methods: {
    ...
    show(cell, left, top) {
      ...
      let itemType = _.get(cell, "source.data.itemType", "");
      if (itemType == "parking-spot")
        return this.createNewParkspot(cell, left, top);
      ...
      this.fetchRelationResClasses(resClass, left, top);
    },
    //获取关系类
    fetchRelationResClasses(resClass, left, top) {
      this.relateNavs = [];
      this.$http({
        method: "GET",
    ...
    createNewParkspot(cell, cx, cy) {
      let bindData = _.get(cell, "source.data.bindData", null);
      if (!bindData) return;
      let label = _.get(cell, "source.data.label", "");
      let resourceClassId = _.get(cell, "source.data.resourceClassId", "");
      let keyName = _.get(cell, "source.data.keyName", "");
      let tableName = _.get(cell, "source.data.tableName", "");
      let width = Number(_.get(cell, "source.data.width", "") || 10);
      let height = Number(_.get(cell, "source.data.height", "") || 10);
      let type = _.get(cell, "source.data.type", "");
      let itemType = _.get(cell, "source.data.itemType", "");
      let rank = _.get(cell, "source.data.rank", "");
      ...
      this.$nextTick(() => {
        ...
        //自动连接新节点
        this.graph.connectCell(cell, vertex, false);
      });
    }
  }
};
