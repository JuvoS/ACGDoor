...
mxVertexToolHandler.prototype.init = function() {
    ...
    let itemType = _.get(this.state.cell, "data.itemType", null);
    const isKpi = itemType === "kpi";

    if (this.hasResClass && !isKpi) {
    ...
    if (this.isBinded) {
        ...
        var alarm = createImage("static/mxgraph/extras/alarm.png");
        alarm.setAttribute("title", "阈值设置");
        alarm.style.cursor = "pointer";
        alarm.style.width = "20px";
        alarm.style.height = "20px";
        alarm.style.margin = "2px";
        mxEvent.addGestureListeners(
            alarm,
            mxUtils.bind(this, function(evt) {
                this.graph.setAlarm(this.state.cell);
                mxEvent.consume(evt);
            })
        );
        this.domNode.appendChild(alarm);
...
export default graph => {
    graph.createHandler = function(state) {
        const flag =
            state != null &&
            !graph.cameraRelateParkingSpaceMode &&
